...
if __name__ == '__main__':
    ...
    # Lists to keep track of losses and accuracies of k fold.
    train_loss_k, valid_loss_k = [], []
    train_acc_k, valid_acc_k = [], []
    
    # Start the training.
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        # Build a new model for every fold
        model = build_model_repvgg(pretrained=False, deploy=False, fine_tune=False, num_classes=3).to(device)
        # model = build_model_effi(pretrained=False, fine_tune=False, num_classes=3).to(device)
        # Optimizer.
        optimizer = optim.Adam(model.parameters(), lr=lr)
        ...
        print(f"[INFO]: Fold {fold+1} of {n_splits}")
    ...
    save_plots(train_acc_k, valid_acc_k, train_loss_k, valid_loss_k, 'Fold')
            
    print('TRAINING COMPLETE')
    print(f'Train acc: {sum(train_acc) / len(train_acc):.3f}')
    print(f'Valid acc: {sum(valid_acc) / len(valid_acc):.3f}')
